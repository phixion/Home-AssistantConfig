  ...
  class CardModder extends cardTools.LitElement {
    ...
    async _cardMod(root) {
      ...
      while (!target) {
        ...
        if (root.querySelector("style")) styles = root.querySelector("style");
        ...
        for (var k in this._config.style) {
          ...
          if (target.style.setProperty) {
            target.style.setProperty(
              k,
              cardTools.parseTemplate(this._config.style[k])
            );
          }
          if (
            this.classList.contains("element") &&
            this.EL_STYLES.indexOf(k) > -1
          ) {
